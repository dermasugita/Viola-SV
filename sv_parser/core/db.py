...
class Sgt_core(object):
    def __init__(self, df_svpos, df_svprof, df_filters, dict_df_info, df_formats, ls_df_headers = []):
        self.df_svpos = df_svpos
        self.df_svprof = df_svprof
        self.df_filters = df_filters
        self.dict_df_info = dict_df_info
        self.df_formats = df_formats
        self.ls_df_headers = df_headers
        ls_keys = ['POSITIONS', 'PROFILES', 'FILTERS'] + list(dict_df_info.keys()) + ['FORMATS'] + \
        ['CONTIGS_META', 'ALTS_META', 'INFOS_META', 'FORMATS_META', 'FILTERS_META']
        ls_values = [df_svpos, df_svprof, df_filters] + list(dict_df_info.values()) + ls_df_headers
