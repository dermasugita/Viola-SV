...
chr3	10	11	chr4	20	21	test12	60	-	-	True
"""
data_expected6 = """chrom1	start1	end1	chrom2	start2	end2	name	score	strand1	strand2	test1
chr1	10	11	chr1	20	21	test1	60	+	-	True
chr1	100	101	chr1	250	251	test3	60	+	-	True
chr1	150	151	chr1	300	301	test5	60	+	-	True
chr2	10	11	chr2	20	21	test7	60	-	-	True
...
chr3	10	11	chr4	20	21	test12	60	-	-	True
"""
data_expected7 = """chrom1	start1	end1	chrom2	start2	end2	name	score	strand1	strand2	test1
chr1	10	11	chr1	25	26	test2	60	+	-	False
chr1	105	106	chr1	290	291	test4	60	+	-	False
chr2	10	11	chr2	20	21	test6	60	-	+	False
chr2	100	101	chr2	280	281	test8	60	-	-	False
...
def test_filter():
    ...
    f_bedpe = bedpe.filter(
        ['svtype == DEL', 'svtype == DUP', 'svlen > -100'],
        query_logic='(0 | 1) & 2')
...
def test_filter_flag():
    bedpe = viola.read_bedpe(StringIO(DATA), patient_name="patient1")
    bedpe_expected = viola.read_bedpe(
        StringIO(data_expected6),
        patient_name='patient1')
    f_bedpe = bedpe.filter(['test1'])
    viola.testing.assert_bedpe_equal(f_bedpe, bedpe_expected)


def test_filter_flag_exception():
    bedpe = viola.read_bedpe(StringIO(DATA), patient_name="patient1")
    bedpe_expected = viola.read_bedpe(
...
def test_filter_flag_exception3():
    ...
    bedpe_expected = viola.read_bedpe(
        StringIO(data_expected7),
        patient_name='patient1')
    f_bedpe = bedpe.filter(['test1 == False'])
    viola.testing.assert_bedpe_equal(f_bedpe, bedpe_expected)
