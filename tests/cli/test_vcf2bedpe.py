...
def test_vcf2bedpe_manta():
  ...
  runner = CliRunner()
  result = runner.invoke(viola, ['vcf2bedpe', path])
...
def test_vcf2bedpe_delly():
  ...
  runner = CliRunner()
  result = runner.invoke(viola, ['vcf2bedpe', '--caller','delly', path])
  assert result.exit_code == 0

def test_vcf2bedpe_lumpy():
  path = os.path.join(HERE, '../io/data/test.lumpy.vcf')
  runner = CliRunner()
  result = runner.invoke(viola, ['vcf2bedpe', '--caller=lumpy', path])
  assert result.exit_code == 0

def test_vcf2bedpe_gridss():
  path = os.path.join(HERE, '../io/data/test.gridss.vcf')
  runner = CliRunner()
  result = runner.invoke(viola, ['vcf2bedpe', '--caller=gridss', path])
  assert result.exit_code == 0
