...
def test_drop_by_id():
    vcf = viola.read_vcf(
        ...
        patient_name='test')
    vcf_dropped_expected = viola.read_vcf(
        os.path.join(HERE, 'data/test.dropped.manta.vcf'),
        variant_caller='manta',
        patient_name='test')
    vcf_dropped2_expected = viola.read_vcf(
        os.path.join(HERE, 'data/test.dropped2.manta.vcf'),
        variant_caller='manta',
        patient_name='test')
...
def test_drop_by_id2():
    vcf = viola.read_vcf2(
        os.path.join(HERE, 'data/test.manta.vcf'),
        variant_caller='manta',
        patient_name='test')
    vcf_dropped_expected = viola.read_vcf2(
        os.path.join(HERE, 'data/test.dropped.manta.vcf'),
        variant_caller='manta',
        patient_name='test')
    vcf_dropped2_expected = viola.read_vcf2(
        os.path.join(HERE, 'data/test.dropped2.manta.vcf'),
        variant_caller='manta',
        patient_name='test')
    vcf_dropped = vcf.drop_by_id('test1')
